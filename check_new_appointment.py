...
def run_visa_scraper(url, no_appointment_text):
    def has_website_changed():
        ...
        if (
            driver.current_url
            == f"https://ais.usvisa-info.com/en-{country_code}/niv/users/sign_in"
        ):
            ...
            try:
                driver.find_element(
                    By.XPATH, "/html/body/div[6]/div[3]/div/button"
                ).click()
            except:
                pass
    ...
    while True:
        ...
        if has_website_changed():
            ...
            logging.info(
                f"No change was found. Checking again in {seconds_between_checks} seconds."
            )
            send_message(
                f"No change was found. Checking again in {seconds_between_checks} seconds.",
                notification_chat_id,
